...
public static class CombatCalculator 
{
        ...
        out bool hasHeavyCover)
    {
        ...
        if (StructureManager.Instance != null)
        {
            ...
            // use attacker & target world positions
            if (StructureManager.Instance.HasStructureCoverBetween(attackerPos, targetPos, out sMed, out sHeavy))
            {
                hasHeavyCover = hasHeavyCover || sHeavy;
                hasMediumCover = hasMediumCover || sMed;
                if (sHeavy) Debug.Log("[Cover] Structure: heavy");
                else if (sMed) Debug.Log("[Cover] Structure: medium");
            }
        }
    ...
    public static List<Unit> GetLineTargetsByWidth(
    Unit caster,
    Vector3 origin,
    Vector3 dir,
    float range,
    ...
    int maxTargets)
    {
        ...
        if (dirLen < 1e-5f) return result;
        dir /= dirLen;

        var all = UnityEngine.Object.FindObjectsByType<Unit>(FindObjectsSortMode.None);
        var picks = new List<(Unit u, float t)>();

        foreach (var u in all)
        {
            if (u == null || u == caster) continue;

            Vector3 rel = u.transform.position - origin;
            rel.z = 0f;

            // forward distance along the line
            float t = Vector3.Dot(rel, dir);
            if (t < 0f || t > range) continue;

            // perpendicular distance to the line
            Vector3 perp = rel - dir * t;
            if (perp.sqrMagnitude > halfWidth * halfWidth) continue;

            picks.Add((u, t));
        }

        // nearest first along the line
        picks.Sort((a, b) => a.t.CompareTo(b.t));

        int cap = Mathf.Max(1, maxTargets);
        for (int i = 0; i < picks.Count && result.Count < cap; i++)
            result.Add(picks[i].u);
